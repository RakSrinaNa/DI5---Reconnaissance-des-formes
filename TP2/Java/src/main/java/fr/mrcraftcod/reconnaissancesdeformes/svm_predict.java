...
class svm_predict{
	private static final Logger LOGGER = LoggerFactory.getLogger(svm_predict.class);
	...
	public static double main(String argv[]) throws IOException{
		...
		try{
			...
			if(model == null){
				LOGGER.error("can't open model file " + argv[i + 1] + "\n");
				System.exit(1);
			}
			if(predict_probability == 1){
			...
			double value = predict(input, output, model, predict_probability);
			input.close();
			output.close();
			return value;
		}
